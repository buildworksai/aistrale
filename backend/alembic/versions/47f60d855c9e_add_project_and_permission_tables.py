...
down_revision: Union[str, Sequence[str], None] = 'f1b539cd6b66'
...
def upgrade() -> None:
    ...
    if "project" not in tables:
        op.create_table(
            ...
            sa.Column("created_at", sa.DateTime(timezone=True), nullable=False, server_default=sa.text("(now() AT TIME ZONE 'utc'::text)")),
    ...
    if "permission" not in tables:
        ...
        op.create_index(op.f("ix_permission_resource_id"), "permission", ["resource_id"], unique=False)
