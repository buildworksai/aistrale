...
class ReliabilityBenchmarkService:
    ...
    def record_metric(self, provider: str, metric: str, value: float):
        ...
        """
        # Find baseline
        baseline = next((b for b in self._benchmarks if b.provider == provider and b.metric == metric), None)
        
        if baseline:
            deviation = ((value - baseline.baseline_value) / baseline.baseline_value) * 100
            if deviation > 20: # 20% degradation threshold
                logger.warning(f"PERFORMANCE ALERT: {provider} {metric} is {value}, {deviation:.1f}% worse than baseline {baseline.baseline_value}")
        
    def get_benchmarks(self, provider: str) -> List[PerformanceBenchmark]:
        return [b for b in self._benchmarks if b.provider == provider]

    def update_baseline(self, provider: str, metric: str, new_values: List[float]):
        """
        ...
        if baseline:
            ...
            logger.info(f"Updated baseline for {provider} {metric} to {avg}")
             ...
             self._benchmarks.append(PerformanceBenchmark(
