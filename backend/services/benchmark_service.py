...
class BenchmarkService:
    ...
    def __init__(self):
        ...
        self.benchmarks = [
            Benchmark(metric="avg_cost_per_token", value=0.002, percentile=50, industry="tech"),
            Benchmark(metric="avg_cost_per_token", value=0.001, percentile=25, industry="tech"), # Lower is better? or just distribution. Lower cost = better percentile usually reversed in cost.
    ...
    def get_benchmark(self, metric: str, industry: str = "tech", percentile: int = 50) -> Optional[Benchmark]:
        """
        Retrieve specific benchmark.
        """
        return next((b for b in self.benchmarks if b.metric == metric and b.industry == industry and b.percentile == percentile), None)

    def compare_to_industry(self, metric: str, user_value: float, industry: str = "tech") -> Dict[str, Any]:
        """
        Compare user value to industry benchmarks.
        Returns context about standing.
        """
        relevant = [b for b in self.benchmarks if b.metric == metric and b.industry == industry]
        if not relevant:
