...
class ABTestService:
    ...
    async def start_test(self, name: str, prompt: str, providers: List[str]) -> ABTest:
        """
        ...
        """
        test = ABTest(name=name, prompt=prompt, providers=providers, status="running")
        # Save to DB (mock)
    ...
    def get_results(self, test_id: int) -> Dict[str, Any]:
        ...
        results = [
            ABTestResult(ab_test_id=test_id, provider="openai", response="A", latency_ms=500, cost=0.02, quality_score=0.9),
            ABTestResult(ab_test_id=test_id, provider="anthropic", response="B", latency_ms=1000, cost=0.01, quality_score=0.95),
        ]
