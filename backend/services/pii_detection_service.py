...
class PIIDetectionService:
    ...
    def __init__(self, languages: List[str] = ["en"]):
        ...
        if not PRESIDIO_AVAILABLE:
            logger.warning("Presidio not available. PII detection will use basic pattern matching.")
            return
            
        try:
            # Try to initialize Presidio, but don't fail if spacy model is missing
            try:
                ...
                logger.warning(f"Presidio initialization failed: {e}. Using basic pattern matching.")
                self.analyzer = None
                self.anonymizer = None
        except Exception as e:
            logger.warning(f"Failed to initialize PIIDetectionService with Presidio: {e}. Using basic pattern matching.")
    ...
    def anonymize(self, text: str, operators: Optional[Dict[str, Any]] = None) -> str:
        """
