...
class KeyRotationService:
    ...
    def _create_initial_key(self) -> EncryptionKey:
        ...
        encrypted_key = master_cipher.encrypt(settings.ENCRYPTION_KEY.encode()).decode()
    ...
    def rotate_key(self) -> Tuple[str, int]:
        ...
        master_key = settings.ENCRYPTION_KEY.encode()
        master_cipher = Fernet(master_key)
        old_key_value = master_cipher.decrypt(old_key.encrypted_key.encode()).decode()
        ...
        encrypted_new_key = master_cipher.encrypt(new_key_value.encode()).decode()
        ...
        for token in tokens:
            try:
                ...
                decrypted_token = old_cipher.decrypt(token.encrypted_token.encode()).decode()
                
                # Encrypt with new key
                token.encrypted_token = new_cipher.encrypt(decrypted_token.encode()).decode()
