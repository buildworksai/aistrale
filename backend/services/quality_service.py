...
class QualityScoringService:
    ...
    def analyze_tradeoff(
        ...
        sorted_by_quality = sorted(
            providers, key=lambda p: p.quality_score, reverse=True
        )
        ...
        if not acceptable:
            best_value = (
                best_quality  # If none meet threshold, best quality is fallback
            )
        else:
            # Best value is lowest cost among acceptable quality
            best_value = min(acceptable,
                             key=lambda p: p.avg_cost_per_1k_tokens)
        ...
        return {
            ...
            "recommendation": (
                best_value.provider
                if best_value.avg_cost_per_1k_tokens
                < best_quality.avg_cost_per_1k_tokens * 0.5
                else best_quality.provider
            ),
        }
