...
def health_service_mock():
    ...
    mock.get_latest_health.return_value = ProviderHealth(
...
async def test_skip_down_provider(health_service_mock):
    ...
    health_service_mock.get_latest_health.side_effect = lambda p: (
        ProviderHealth(provider=p, status="down")
        if p == "openai"
        else ProviderHealth(provider=p, status="healthy")
    )
