...
async def test_run_evaluation(mock_session: Session):
    ...
    def get_side_effect(model, id):
        if model == Evaluation: return evaluation
        if model == Prompt: return prompt
        return None
    ...
    # Mock load_dataset
    service.load_dataset = MagicMock(return_value=[
    ...
    with patch("services.inference_service.run_inference") as mock_run_inference:
        ...
        # If using MagicMock, we need to set return_value to a future or use side_effect async.
        ...
        assert mock_session.add.call_count >= 3 # eval update + 2 results + eval completion
