...
class TestInferenceComplete:
    ...
    async def test_run_inference_all_paths(
            self, authenticated_client, mock_session):
...
class TestTelemetryComplete:
    ...
    def test_read_telemetry_all_paths(
            self, authenticated_client, mock_session):
    ...
    def test_get_cost_analytics_all_paths(
        ...
        response = authenticated_client.get(
            "/api/telemetry/cost-analytics?provider=openai"
        )
        assert response.status_code in [200, 401, 500]
        ...
        # Group by week
        response = authenticated_client.get(
            "/api/telemetry/cost-analytics?group_by=week"
        )
        assert response.status_code in [200, 401, 500]

        # Group by month
        response = authenticated_client.get(
            "/api/telemetry/cost-analytics?group_by=month"
        )
        assert response.status_code in [200, 401, 500]

        # Group by provider
        response = authenticated_client.get(
            "/api/telemetry/cost-analytics?group_by=provider"
        )
        assert response.status_code in [200, 401, 500]

        # Group by model
        response = authenticated_client.get(
            "/api/telemetry/cost-analytics?group_by=model"
        )
        assert response.status_code in [200, 401, 500]
...
class TestPromptsComplete:
    ...
    def test_create_prompt_all_paths(self, authenticated_client, mock_session):
        ...
        existing_prompt = Prompt(
            id=1, name="test", template="Hello", user_id=1, version=1
        )
    ...
    def test_update_prompt(self, authenticated_client, mock_session):
        ...
        response = authenticated_client.patch(
            "/api/prompts/1", json={"template": "Updated"}
        )
        assert response.status_code in [200, 401, 404, 500]
...
class TestSecurityAuditComplete:
    ...
    def test_list_audit_events_all_paths(self, admin_client, mock_session):
        ...
        response = admin_client.get(
            "/api/security-audit/?event_type=login_success")
        assert response.status_code in [200, 401, 403, 500]
...
class TestAdminEndpoints:
    ...
    def test_rotate_encryption_key(self, admin_client, mock_session):
        ...
        with patch.object(
            KeyRotationService, "rotate_key", return_value=("new-key-id", 5)
        ):
    ...
    def test_rotate_key_error_handling(self, admin_client, mock_session):
        ...
        with patch.object(
            KeyRotationService, "rotate_key", side_effect=Exception("Test error")
        ):
            response = admin_client.post("/api/admin/rotate-encryption-key")
