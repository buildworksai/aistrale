...
def mock_limiter():
    ...
    with patch("api.auth.limiter") as mock_auth_limiter, patch(
        "api.tokens.limiter"
    ) as mock_tokens_limiter, patch("api.inference.limiter") as mock_inference_limiter:
...
class TestTokensFinal:
    ...
    def test_create_token_default_flow(
    ...
    @patch("api.tokens.get_current_user_id")
    def test_delete_token_audit_logging(
            self, mock_user_id, client, mock_session):
    ...
    @patch("api.tokens.get_current_user_id")
    def test_set_default_unset_others(
            self, mock_user_id, client, mock_session):
...
class TestInferenceFinal:
    ...
    async def test_run_inference_complete(
        ...
        history_msg = ChatMessage(
            id=1, user_id=1, role="user", content="Previous")
...
class TestSecurityAuditFinal:
    ...
    def test_list_audit_all_filters(
            self,
            mock_require_admin,
            client,
            mock_session):
