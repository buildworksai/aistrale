...
class TestSecurityAuditAPI:
    ...
    def test_list_audit_events_as_admin(
            self, client, mock_session, test_audit_events):
    ...
    def test_list_audit_events_with_event_type_filter(
        self, client, mock_session, test_audit_events
    ):
        ...
        filtered_events = [
            e for e in test_audit_events if e.event_type == "login_success"
        ]
        mock_result = MagicMock()
    ...
    def test_list_audit_events_with_user_filter(
        self, client, mock_session, test_audit_events
    ):
    ...
    def test_list_audit_events_with_date_filter(
        self, client, mock_session, test_audit_events
    ):
        ...
        filtered_events = [
            e for e in test_audit_events if start_date <= e.created_at <= end_date]
        mock_result = MagicMock()
        mock_result.all.return_value = filtered_events
        mock_session.exec.return_value = mock_result
