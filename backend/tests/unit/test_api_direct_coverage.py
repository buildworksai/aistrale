...
class TestPromptsDirect:
    ...
    def test_update_prompt_direct(self, client, mock_session):
        ...
        response = client.patch("/api/prompts/1", json={"template": "Updated"})
...
class TestInferenceDirect:
    ...
    async def test_run_inference_direct(self, client, mock_session):
        ...
        with patch("api.inference.Request") as mock_request:
            ...
            with patch(
                "api.inference.run_inference", new_callable=AsyncMock
            ) as mock_run:
    ...
    @pytest.mark.asyncio
    async def test_run_inference_token_not_found_direct(
    ...
    @pytest.mark.asyncio
    async def test_run_inference_provider_mismatch_direct(
            self, client, mock_session):
