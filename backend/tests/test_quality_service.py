...
def test_analyze_tradeoff():
    ...
    p1 = ProviderPerformance(provider="cheap", model="v1", quality_score=0.85, avg_cost_per_1k_tokens=0.01)
    p2 = ProviderPerformance(provider="expensive", model="v2", quality_score=0.95, avg_cost_per_1k_tokens=0.10)
    p3 = ProviderPerformance(provider="bad", model="v3", quality_score=0.50, avg_cost_per_1k_tokens=0.005)
    ...
    assert analysis["best_value"]["provider"] == "cheap" # 0.85 > 0.8 and much cheaper
    
    # Recommendation should favor value because cheap is 10x cheaper (0.01 vs 0.10) which is < 0.5 * 0.10
